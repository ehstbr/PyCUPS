...
class MappedWindowPresenter:
    """Present a child only after its intended parent is mapped."""
    ...
    def clear(self) -> None:
        if self._parent is not None and self._map_handler is not None:
            try:
                self._parent.disconnect(self._map_handler)
