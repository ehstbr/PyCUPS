...
VERSION_MANIFEST_URL = "https://raw.githubusercontent.com/ehstbr/PyCUPS/main/version.json"
RELEASES_URL = "https://github.com/ehstbr/PyCUPS/releases/latest"
...
class SemanticVersion:
    ...
    def parse(cls, value: str) -> SemanticVersion:
        ...
        prerelease = cls._parse_identifiers(prerelease_text if dash else "", prerelease=True)
...
class UpdateManifest:
    ...
    def released_at_utc_text(self) -> str:
        return self.released_at.astimezone(timezone.utc).strftime("%Y-%m-%d %H:%M UTC")
...
def parse_update_manifest(data: bytes) -> UpdateManifest:
    ...
    try:
        ...
        raise UpdateManifestError("The update manifest version is invalid.") from error
    ...
    for item in changelog_value:
        ...
        item = item.strip()
        if len(item) > MAX_CHANGELOG_ITEM_CHARACTERS:
    ...
    if not isinstance(released_at_text, str) or not _UTC_TIMESTAMP.fullmatch(released_at_text):
    ...
    try:
        ...
        raise UpdateManifestError("The release date is invalid.") from error
...
def evaluate_update(data: bytes, *, current_version: str = VERSION) -> UpdateCheckResult:
    latest = parse_update_manifest(data)
    try:
        installed = SemanticVersion.parse(current_version)
    except ValueError as error:
        raise UpdateManifestError("The installed application version is invalid.") from error
    return UpdateCheckResult(latest=latest, update_available=latest.version > installed)
...
class UpdateChecker:
    def __init__(self, *, http: Any | None = None, url: str = VERSION_MANIFEST_URL) -> None:
        if http is None:
    ...
    def check(self, callback: UpdateCallback, *, current_version: str = VERSION) -> None:
        self.cancel()
        ...
        def finished(status: int, data: bytes, error: Exception | None) -> None:
            ...
            except UpdateManifestError as manifest_error:
                callback(UpdateCheckResult(error=str(manifest_error)))
