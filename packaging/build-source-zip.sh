...
source_date_epoch="${SOURCE_DATE_EPOCH:-1787230410}"

for command_name in find grep mkdir mktemp rm sed sort tar touch zip msgfmt; do
...
work_dir="$(mktemp -d "${TMPDIR:-/tmp}/print-archive-source.XXXXXX")"
cleanup() {
    case "$work_dir" in
        /tmp/print-archive-source.*|"${TMPDIR:-/tmp}"/print-archive-source.*) rm -rf -- "$work_dir" ;;
    esac
}
trap cleanup EXIT

archive_root="PyCUPS-$version"
